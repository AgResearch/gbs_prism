...
def _kgd_job_spec(
    ...
    return JobNSpec(
        ...
        expected_paths={
            ...
            HEATMAP_ORDER_HWDGM05_CSV: os.path.join(out_dir, "HeatmapOrderHWdgm.05.csv"),
            HIGH_RELATEDNESS_CSV: os.path.join(out_dir, "HighRelatedness.csv"),
            HIGH_RELATENESS_SPLIT_CSV: os.path.join(out_dir, "HighRelatedness.split.csv"),
            ...
            KGD_STDOUT: os.path.join(out_dir, "KGD.stdout"),
        },
        expected_globs={},
    )
...
def kgd(work_dir: str, genotyping_method: str, hap_map_files: list[File]) -> KgdOutput:
    ...
    return KgdOutput(
        ...
        heatmap_order_hwdgm05_csv=result_files.expected_files[HEATMAP_ORDER_HWDGM05_CSV],
        high_relatedness_csv=result_files.expected_files[HIGH_RELATEDNESS_CSV],
        high_relateness_split_csv=result_files.expected_files[HIGH_RELATENESS_SPLIT_CSV],
        seq_ID_csv=result_files.expected_files[SEQ_ID_CSV],
        sample_stats_raw_csv=result_files.expected_files[SAMPLE_STATS_RAW_CSV],
        sample_stats_raw_combined_csv=result_files.expected_files[SAMPLE_STATS_RAW_COMBINED_CSV],
