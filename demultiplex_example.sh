...
#
# The script below extracts a multi-library keyfile containing all GBS samples for two species (takahe and kakapo), and 
# does a demultiplexing and KGD run for each distinct enzyme
#
set -x
...
mkdir $WORKDIR 
echo 'MergeTaxaTagCount -t n' > $WORKDIR/demultiplexing_parameters.txt # this option keeps lanes distinct which we do for novaseq


# for each distinct enzyme set up and run demultiplex and KGD 
for enzyme in `gquery -t gbs_keyfile -b taxname -p "columns=enzyme;distinct;noheading" takahe kakapo`; do
   mkdir -p $WORKDIR/$enzyme

   # get keyfile for this enzyme. Note that we need to use the "qc_sampleid" sample-names, as the native sample names  for these have embedded spaces and other features
   # which causes processing problems downstream. (See below where an "unblinding" script is extracted, that can be used to edit the text output files to map back to the original sampleid  
   # (for other cases, such as deer or goats, you could probably just used the original sampleid, so the gquery extract command would be 
   ...
   # get a "sed" script that can be used to unblind the results
