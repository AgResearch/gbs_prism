{
  ...
  inputs = {
    ...
    kgd = {
      url = "github:AgResearch/KGD?ref=refs/tags/v1.4.1";
      inputs.nixpkgs.follows = "nixpkgs";
    };
    GUSbase = {
          ...
          gbs-prism-bundle =
            ...
            pkgs.stdenv.mkDerivation {
              ...
              installPhase = ''
                ...
                cat >$out/bin/run_gbs_prism <<EOF
                #!{pkgs.bash}/bin/bash

                test \$# -eq 1 || { echo >&2 "usage: run_gbs_prism <run>"; exit 1; }
                test "\$1" == "--help" -o "\$1" == "-h" && { echo >&2 "usage: run_gbs_prism <run>"; exit; }
                exec $out/bin/redun run "\$GBS_PRISM/pipeline.py" main --run "\$1"
                EOF
                chmod 555 $out/bin/run_gbs_prism
